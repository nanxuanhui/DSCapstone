...
# Ensure ONNX is installed before running
try:
    import onnx
except ImportError:
    print("ONNX not found, attempting to install...")
    os.system("pip install onnx onnxruntime onnx-simplifier")
    try:
        import onnx
        print("ONNX installation successful!")
    except ImportError:
        print("ONNX installation failed. Skipping ONNX-related tasks.")
        onnx = None
...
def quantize_yolo():
    if onnx is None:
        print("Skipping YOLOv5 ONNX quantization due to missing ONNX module.")
        return
